#Proof of concept implementation of 17-round key recovery attack of simeck32/64
...
def test_bayes(cts, real_key,net, net_help, 
    ...
    for j in range(it):
        ...
        if (vtmp > cutoff1):
            ...
            for i2 in l2:
                c0a, c1a = cipher.dec_one_round_simeck((cts[0][i][:16384],cts[1][i][:16384]),keys[i2])
                c0b, c1b = cipher.dec_one_round_simeck((cts[2][i][:16384],cts[3][i][:16384]),keys[i2])         
...
def test(n):
    ...
    net_name='staged_simeck32_best_model_12r_depth5_num_epochs10_pairs8_acc_0.5156474709510803.h5'
    ...
    path='./WKRP/'
    m_main=np.load(path+"simeck32_data_wrong_key_mean_12r_pairs8.npy")
    s_main=np.load(path+"simeck32_data_wrong_key_std_12r_pairs8.npy")
    s_main=1.0/s_main
        
    m_help=np.load(path+"simeck32_data_wrong_key_mean_11r_pairs8.npy")
    s_help=np.load(path+"simeck32_data_wrong_key_std_11r_pairs8.npy")
    s_help=1.0/s_help
